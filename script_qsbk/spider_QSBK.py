...
'''
blog: http://cuiqingcai.com/990.html
'''
...
class QSBK:
	...
	def __init__(self):
		self.pageIndex = 1
		self.user_agent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36'
		#初始化headers
	...
	def start(self):
		...
		while self.enable:
			if len(self.stories)>0:
				...
				del self.stories[0]
				#输出该页的段子
				print 'nowPage {}, stories {}'.format(nowPage, len(pageStories))
				self.getOneStory(pageStories,nowPage)
				self.loadPage()
